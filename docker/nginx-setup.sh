...
server {
  ...
  # agentgui sync WebSocket: browsers cannot send Authorization: Basic on a WS
  # handshake, so auth_basic must be off here; agentgui authenticates the socket
  # via ?token=<PASSWORD> (lib/ws-setup.js). Routes both /sync and /gm/sync.
  location = /sync {
    auth_basic off;
...
# Test /desk endpoint (must now ENFORCE HTTP Basic Auth from PASSWORD, expect 401)
